...
import com.cisco.poc.service.ElastiSearchService;
...
public class ElasticSearchCRUDTest extends BaseTest {
	    ...
	    public void findDocumentByValue() {
	        SearchResponse response = elastiSearchService.findDocument("users", "user", "LastName", "Gaurav");
	    ...
	    public void UpdateDocument() throws IOException {
	        XContentBuilder jsonBuilder = XContentFactory.jsonBuilder();
	        Map<String, Object> data = new HashMap<String, Object>();
	        data.put("FirstName", "Rayaan");
	        data.put("LastName", "Jha");
	        jsonBuilder.map(data);
	        UpdateResponse updateResponse = elastiSearchService.updateIndex("users", "user", "1", jsonBuilder);
	        SearchResponse response = elastiSearchService.findDocument("users", "user", "LastName", "Jha");
	        Assert.assertSame(response.getHits().totalHits() > 0, true);
	    }
	    ...
	    public void RemoveDocument() throws IOException {
	        ...
	        Assert.assertSame(response.isExists(), false);
	    }
	}
